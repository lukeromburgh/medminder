...
class Tier(models.Model):
    ...
    priority = models.IntegerField(
        default=0, help_text="Lower number means higher priority/better tier"
    )  # Optional: for ordering tiers
    is_default = models.BooleanField(
        default=False, help_text="Designates if this is the default tier for new users."
    )
...
class UserSettings(models.Model):
    ...
    )  # Use related_name for easy access from User
    ...
    avatar_text_color = models.CharField(
        max_length=50,
...
def create_user_settings(sender, instance, created, **kwargs):
    if created:
        ...
        UserSettings.objects.create(
            user=instance,
            account_tier=default_tier,
            subscription_status="free",
            avatar_bg_color=bg_color,
            avatar_text_color=text_color,
        )  # Assign default tier and status on creation
...
class ReceiveUpdates(models.Model):
    ...
    notifications = models.BooleanField(
        default=True, help_text="Receive notifications about updates and new features."
    )
