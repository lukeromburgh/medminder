...
class ScheduleForm(forms.Form):
    ...
    at_time = forms.TimeField(
        label='At',
        widget=forms.TimeInput(format='%H:%M', attrs={ # Use TimeInput for consistent format
            'class': 'mt-1 block w-full rounded-md border border-gray-300 p-2 focus:border-gray-500 focus:ring-gray-500 sm:text-sm',
    ...
    until_date = forms.DateField(
        label='Until',
        widget=forms.DateInput(format='%Y-%m-%d', attrs={ # Use DateInput for consistent format
            'class': 'mt-1 block w-full rounded-md border border-gray-300 p-2 focus:border-gray-500 focus:ring-gray-500 sm:text-sm',
        ...
        if repeat == 'weekly':
            ...
            try:
                ...
                if not all(0 <= day <= 6 for day in days):
                     raise forms.ValidationError("Invalid day number provided. Please select days from the options.")
        ...
        if repeat == 'monthly':
            ...
            try:
                ...
                if not dates: # Handle case where split resulted in no valid dates
                     raise forms.ValidationError("Please select at least one valid date for monthly repetition.")
    ...
    def clean(self):
        ...
        if repeat == 'weekly':
            ...
            if weekly_days_str: # weekly_days_str is already validated by clean_weekly_days
                 try:
                     ...
                     self.add_error('weekly_days', "Internal error processing weekly days.")
            ...
            if monthly_dates_str: # monthly_dates_str is already validated by clean_monthly_dates
                try:
                    ...
                    self.add_error('monthly_dates', "Internal error processing monthly dates.")
        ...
        if start_date and until_date and until_date < start_date:
            self.add_error('until_date', "The end date cannot be before the start date.")
