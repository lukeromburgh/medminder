...
class DailyReminderLog(models.Model):
    ...
    )  # Use string 'Reminder' if defined later in file
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE)
    due_date = models.DateField(db_index=True)
    due_time = models.TimeField()
    is_notified = models.BooleanField(
        default=False
    )  # For tracking push/email notifications
    ...
    def is_past_grace_period(self):
        ...
        grace_period = getattr(
            self.reminder, "grace_period", timedelta(minutes=15)
        )  # Default grace if not on reminder
...
class Viewer(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="viewed_user"
    )
    viewer_user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="viewer"
    )
