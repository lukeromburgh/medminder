...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Define a time window for reminders, e.g., due within the next 15 minutes
        time_window_end = now + timedelta(minutes=15)

        self.stdout.write(f"Checking for reminders due between {now.time().strftime('%H:%M')} and {time_window_end.time().strftime('%H:%M')} today...")

        # Find pending DailyReminderLog entries that are:
        # 1. For today's date
        # 2. Due within the next time window
        # 3. Have not been notified yet
        # 4. Belong to users who have email reminders enabled
        due_reminders = DailyReminderLog.objects.filter(
            due_date=now.date(),
            due_time__gte=now.time(),
            due_time__lte=time_window_end.time(),
            status='pending',
            is_notified=False,
            user__usersettings__receive_email_reminders=True # Check user setting
        ...
        for log_entry in due_reminders:
            ...
            # template = render_to_string('reminders/email/reminder_email.html', {
            #     'log_entry': log_entry,
            #     'user': user,
            # })
            # Or use a simple plain text message
            message = (
                f"Hi {user.username},\n\n"
