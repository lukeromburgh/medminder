...
from ....accounts.models import UserSettings # Import UserSettings model (assuming this path)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"Checking for reminders due between {now.strftime('%Y-%m-%d %H:%M:%S %Z')} and {time_window_end.strftime('%Y-%m-%d %H:%M:%S %Z')}...")

        # Base filters for pending, not notified, and email enabled users
        base_filters = Q(status=DailyReminderLog.STATUS_PENDING) & Q(is_notified=False) & Q(user__usersettings__receive_email_reminders=True)
        ...
        if time_window_end.date() > now.date():
             ...
             self.stdout.write(f"  Checking logs due today ({now.date()}) and tomorrow ({time_window_end.date()}).")
        else:
        ...
        ).select_related('user', 'reminder__medication', 'reminder__dosage', 'reminder__schedule') # Select related for efficiency

        due_reminders = []
        # Iterate and filter based on the full timezone-aware due_datetime
        for log_entry in potential_reminders:
            due_datetime_aware = log_entry.due_datetime # Use the timezone-aware property
        ...
        for log_entry in due_reminders:
            ...
            if log_entry.status != DailyReminderLog.STATUS_PENDING or log_entry.is_notified:
                 self.stdout.write(f"Skipping log entry {log_entry.id} - status not pending or already notified during final check.")
            ...
            try:
                ...
                log_entry.save(update_fields=['is_notified']) # Use update_fields for efficiency
                sent_count += 1
