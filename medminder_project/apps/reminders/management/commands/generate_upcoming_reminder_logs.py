...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Starting log generation for active reminders up to {future_window_end}..."
            )
        )
        ...
        for reminder in active_reminders:
            ...
            if schedule.end_date and schedule.end_date < today:
                ...
                # reminder.save()
                self.stdout.write(
                    f"Skipping reminder {reminder.id} as its end date ({schedule.end_date}) is in the past."
                )
                continue
            ...
            if generation_start_date > generation_end_date:
                self.stdout.write(
                    f"Skipping reminder {reminder.id} as generation window is invalid ({generation_start_date} to {generation_end_date})."
                )
                continue

            self.stdout.write(
                f"Generating logs for reminder {reminder.id} from {generation_start_date} to {generation_end_date}"
            )
            ...
            expected_occurrences = self.generate_expected_occurrences(
            ...
            for occurrence_date in expected_occurrences:
                ...
                if created:
                    self.stdout.write(
                        f"  Created log for {reminder.id} on {occurrence_date}"
                    )
        ...
        while current_date <= end_date:
            ...
            if schedule.repeat_type == "daily":
                ...
                selected_days_of_month = [
                    datetime.date.fromisoformat(d).day
                    for d in schedule.monthly_dates.split(",")
                    if d
                ]
            ...
            if schedule.end_date and current_date > schedule.end_date:
                is_an_occurrence = (
                    False  # Should already be handled by loop bounds, but good failsafe
                )
