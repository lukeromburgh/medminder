...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        user_ids = DailyReminderLog.objects.values_list(
            "user_id", flat=True
        ).distinct()  # Use user_id

        for user_id in user_ids:
            completed_yesterday = DailyReminderLog.objects.filter(
                user_id=user_id, due_date=yesterday, status="completed"
            ).exists()
            ...
            if completed_yesterday:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"ReminderStats updated for user {user_id} on {yesterday}: Completed"
                    )
                )
            else:
                self.stdout.write(
                    self.style.SUCCESS(
                        f"ReminderStats updated for user {user_id} on {yesterday}: Not Completed"
                    )
                )

        self.stdout.write(
            self.style.SUCCESS("Successfully populated/updated ReminderStats.")
        )
