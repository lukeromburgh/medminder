...
from django.db import models # Import models for formfield_for_foreignkey if needed (unlikely with autocomplete)
...
class ScheduleAdmin(ModelAdmin):
    ...
    list_display = ('repeat_type', 'time_of_day', 'start_date', 'end_date_display', 'weekly_days', 'monthly_dates')
...
class ReminderAdmin(ModelAdmin):
    ...
    list_display = ('user', 'medication', 'dosage', 'schedule', 'is_active', 'created_at')
    ...
    raw_id_fields = ('user', 'medication', 'dosage', 'schedule') # Good for performance with many related objects
...
class DailyReminderLogAdmin(ModelAdmin):
    ...
    raw_id_fields = ('reminder', 'user') # Or use autocomplete_fields = ('reminder', 'user') if configured
    ...
    medication_name.admin_order_field = 'reminder__medication__medication_name' # Allows sorting
    ...
    def effective_status_display(self, obj):
        ...
        color = 'green' if effective_stat in [obj.STATUS_COMPLETED, obj.STATUS_TAKEN_LATE] else ('red' if effective_stat == obj.STATUS_MISSED else 'orange')
        return format_html('<span style="color: {}; font-weight: bold;">{}</span>', color, obj.get_effective_status_display())
...
class UserStatsAdmin(ModelAdmin):
    ...
    list_display = ('user', 'date', 'reminders_completed', 'reminders_skipped', 'average_compliance', 'achievement_points')
...
class ReminderStatsAdmin(ModelAdmin):
    ...
    list_filter = ('date', 'completed', 'skipped', 'reminder__medication', 'reminder__user') # Filter by related fields
    date_hierarchy = 'date'
