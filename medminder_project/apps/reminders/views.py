...
TEMPLATES = {
    "medication": "reminders/forms/medication_form.html",
...
class ReminderWizard(SessionWizardView):
    ...
    def done(self, form_list, **kwargs):
        ...
        if not all_data:
            print(
                "--- Error in done method: Cleaned data is empty. Redirecting to add_reminder. ---"
            )
        ...
        try:
            ...
            if (
                reminder.is_active
                and reminder.schedule.start_date <= today
                and (
                    reminder.schedule.end_date is None
                    or today <= reminder.schedule.end_date
                )
            ):
                ...
                if schedule.repeat_type == "daily":
                    ...
                    try:
                        weekly_days_list = [
                            int(day.strip())
                            for day in schedule.weekly_days.split(",")
                            if day.strip()
                        ]
                    ...
                    try:
                        monthly_dates_list = [
                            int(day.strip())
                            for day in schedule.monthly_dates.split(",")
                            if day.strip()
                        ]
                        if (
                            1 <= today_day_of_month <= 31
                            and today_day_of_month in monthly_dates_list
                        ):
                            is_due_today = True
                    except ValueError:
                        pass
...
def all_medications(request):
    ...
    all_reminders = (
        ...
        .select_related("medication", "dosage", "schedule")
        .order_by("medication__medication_name", "schedule__time_of_day")
    )
    ...
    return render(
        request, "reminders/medications.html", context
    )  # Changed template name to match our previous step
...
def dashboard_today(request):
    ...
    now = timezone.localtime(
        timezone.now()
    ).time()  # Use timezone.localtime() for the user's local time
    user = request.user
    ...
    for reminder in active_reminders_due_today_or_later:
        ...
        is_within_end_date_range = (
            schedule.end_date is None or today <= schedule.end_date
        )
    ...
    todays_reminders = (
        DailyReminderLog.objects.filter(
            user=user,
            due_date=today,
        )
        .select_related(
    ...
    next_reminder = (
        upcoming_logs_query.select_related(
            "reminder__medication", "reminder__dosage", "reminder__schedule"
        )
    ...
    logs_last_week = DailyReminderLog.objects.filter(
        user=user, due_date__gte=one_week_ago, due_date__lte=today  # Include today
    )
    ...
    weekly_adherence = (
        int((completed_last_week / total_due_last_week * 100))
        if total_due_last_week > 0
        else 100
    )
    ...
    user_tier = get_user_tier(
        achievement_points
    )  # Assuming you have a get_tier_display method or similar
    ...
    logs_for_tracker = DailyReminderLog.objects.filter(
        user=user, due_date__gte=start_date_year, due_date__lte=today
    ).values(
        "due_date", "status"
    ...
    for log in logs_for_tracker:
        ...
        if log["status"] == "completed":
            ...
            log["status"] == "pending"
        ):  # Treat past pending as missed for visualization
    ...
    while current_date <= today:
        ...
        if current_date in logs_by_date:
            ...
            if (
                day_data["pending"] > 0 and current_date < today
            ):  # Past day with pending logs = missed
...
def delete_reminder(request, reminder_id):
    ...
    if reminder.user != request.user:
        messages.error(
            request, "You don't have permission to delete this medication plan."
        )
    ...
    messages.success(
        request, f"'{medication_name}' medication plan has been deleted successfully."
    )
...
def complete_reminder(request, reminder_id):
    ...
    if request.method == "POST":
        ...
        if log_entry.status == DailyReminderLog.STATUS_PENDING:
            ...
            completion_type = (
                "Completed"
                if log_entry.status == DailyReminderLog.STATUS_COMPLETED
                else "Taken Late"
            )
            ...
            points_before_this_action = (
                user_stats.achievement_points - total_points_added
            )
            ...
            new_tier_name, new_badge_image = get_user_tier(
                user_stats.achievement_points
            )
            ...
            response_data = {
                ...
                "action_time": (
                    log_entry.action_timestamp.strftime("%H:%M")
                    if log_entry.action_timestamp
                    else None
                ),
            }
            ...
            if old_tier_name != new_tier_name:
                ...
                response_data["rank_up_date"] = (
                    timezone.now().strftime("%d %b %Y").upper()
                )  # e.g., 29 APR 2025
        ...
        # Reminder was not in 'pending' state
            ...
            return JsonResponse(
                ...
                status=202,
            )  # Bad request - cannot complete non-pending item
...
def dashboard_calendar(request):
    ...
    print(
        f"User settings: subscription_status={user_settings.subscription_status}, account_tier={user_settings.account_tier}"
    )
    ...
    if user_settings.subscription_status == "premium":
        ...
        queryset_1 = Reminder.objects.filter(
            user=user,
            is_active=True,
            schedule__start_date__lte=today
            + timedelta(days=30),  # Adjust the range as needed
        ...
        upcoming_reminders = queryset_1.union(queryset_2).order_by(
            "schedule__start_date", "schedule__time_of_day"
        )
        ...
        for reminder in upcoming_reminders:
            ...
            if schedule.repeat_type == "daily":
                ...
                while current_date <= (
                    end_date if end_date else today + timedelta(days=30)
                ):
                    events.append(
                        {
                ...
                days_of_week = [
                    int(day) for day in schedule.weekly_days.split(",") if day.strip()
                ]
                current_date = start_date
                while current_date <= (
                    end_date if end_date else today + timedelta(days=30)
                ...
                dates_of_month = [
                    int(day) for day in schedule.monthly_dates.split(",") if day.strip()
                ]
                current_date = start_date
                while current_date <= (
                    end_date if end_date else today + timedelta(days=30)
...
def account_page_view(request):
    ...
    try:
        ...
        user_tier = get_user_tier(
            achievement_points
        )  # Assuming get_user_tier returns (name, image_url)
        user_tier_name = user_tier[0]
        badge_image_url = user_tier[1]
        current_streak_count = calculate_current_adherence_streak(
            user
    ...
    return render(
        request, "reminders/account_page.html", context
...
def update_user_settings(request):
    ...
    if is_ajax_request:
        ...
        try:
            ...
            user_settings.receive_email_reminders = (
                request.POST.get("receive_email_reminders") == "on"
            )
        ...
        return JsonResponse(
            {"success": False, "errors": "This endpoint only accepts AJAX requests."},
            status=400,
        )
...
def manage_plan(request):
    ...
    return redirect(
        "medminder:account"
    )  # Assuming 'account' is the name for the user account page URL pattern
