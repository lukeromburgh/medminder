    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not request.user.is_authenticated:
                ...
                form = ReceiveUpdatesForm(
                    initial={"email": email}
                )  # Keep email in the form
            ...
            if existing_entry:
                ...
                ReceiveUpdates.objects.create(
                    user=request.user, email=email, notifications=True
                )
        ...
        if request.user.is_authenticated:
            try:
                ...
                messages.info(
                    request, "You are already signed up for updates!"
                )  # Inform them they are already signed up
